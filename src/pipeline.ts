  ...
  try {
    ...
    for (const episode of pendingTranscripts) {
      ...
      try {
        const result = (await fetchTranscript(
          ...
          status: "stored" | "already-stored" | "no-transcript" | "preflight-blocked" | "quarantined";
        };
    ...
    if (config.exportDir) {
      const exportTargets = Object.values(stateAfterTranscripts.episodes).filter(
        (episode) => episode.transcriptStatus === "stored",
      );
    ...
    return {
      ...
      ...(lockLost
        ? {
            aborted: "lock-lost",
            note:
              "The run lock was lost mid-run (released or broken by an explicit recovery while " +
