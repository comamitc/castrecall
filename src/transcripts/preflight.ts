 ...
 * assurance.
 */
export function classifyWhisperModelQuality(
  flavor: WhisperFlavor | undefined,
  resolved: WhisperModelResolution,
): WhisperModelQuality {
  if (resolved.model) {
    if (APPROVED_MODEL_PATTERN.test(resolved.model)) return "approved";
    if (LOW_QUALITY_MODEL_PATTERN.test(resolved.model)) return "low-quality";
    return "unknown";
  }
  ...
  if (flavor === "mlx-whisper") return "low-quality";
  return "unknown";
}

const RUNTIME_CAVEAT =
  "Rough class only — no audio durations are known ahead of time; actual runtime depends on " +
  "episode length, hardware, and model size.";

export function estimateRuntimeClass(
  episodesPendingTranscript: number,
  backend: WhisperFlavor | null,
): { runtimeClass: string; runtimeCaveat: string } {
  if (backend === null) {
...
export type TranscriptionPreflight = {
  ...
  timestamps: { segments: boolean; words: boolean };
  ...
  sttFallback: { enabled: boolean; provider: SttProvider; available: boolean };
