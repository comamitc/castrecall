...
export function scoreTranscriptQuality(
  input: { text: string; source: LadderRung; segments?: TranscriptSegment[] },
  ...
  if (segments.length > 0) {
    const suspicious = segments.filter(
      (segment) => segment.text.trim() === "" || segment.text.length > t.MAX_SEGMENT_CHARS,
    ).length;
  ...
  const tier: QualityTier = loop.looped
    ? "search-only"
