...
export type SecretBackend =
  | { kind: "macos-keychain"; bin: string }
  | { kind: "libsecret"; bin: string };

export type SecretBackendDetection =
  | { backend: SecretBackend; reason?: undefined }
...
export async function detectSecretBackend(
  ...
  if (platform === "darwin") {
    ...
    if (bin) return { backend: { kind: "macos-keychain", bin } };
    return { reason: "macOS Keychain CLI ('security') not found on PATH." };
  }
  if (platform === "linux") {
    const bin = await findOnPath("secret-tool", env.PATH ?? "");
    if (bin) return { backend: { kind: "libsecret", bin } };
    return {
      reason:
        "libsecret's 'secret-tool' not found on PATH. Install it (e.g. 'apt install " +
...
export async function writeSecret(
  ...
  if (backend.kind === "macos-keychain") {
    ...
    const result = await execImpl(
      [backend.bin, "add-generic-password", "-U", "-s", service, "-a", account, "-w", value],
      { timeoutMs: SECRET_TIMEOUT_MS },
    );
  ...
  const result = await execImpl(
    [backend.bin, "store", "--label", `CastRecall ${account}`, "service", service, "account", account],
    { timeoutMs: SECRET_TIMEOUT_MS, stdin: value },
  );
...
function defaultExec(argv: string[], options: { timeoutMs: number; stdin?: string }): Promise<ExecResult> {
