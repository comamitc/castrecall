...
export async function login(
  ...
  if (response.status === 401 || response.status === 403) {
    throw new PocketCastsAuthError(
      "Pocket Casts rejected the configured credentials. Check POCKETCASTS_EMAIL and " +
  ...
  if (!response.ok) {
    throw new PocketCastsApiError(
      `Pocket Casts login failed with HTTP ${response.status}. The unofficial API may have changed.`,
      response.status,
    );
  }
...
export async function fetchHistory(
  ...
  if (!response.ok) {
    throw new PocketCastsApiError(
      `Pocket Casts history request failed with HTTP ${response.status}.`,
      response.status,
    );
  }
