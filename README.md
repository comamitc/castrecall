...
| `CASTRECALL_WHISPER_WORD_TIMESTAMPS` | no | `true` for word-level timestamps where the flavor supports it. Ignored on whisper.cpp (no boolean flag; only its full-JSON `-ojf` output has word-level timing). |
...
| `CASTRECALL_WHISPER_LOGPROB_THRESHOLD` | no | Numeric average-logprob threshold below which a segment is treated as failed. Not supported by whisper.cpp (ignored with a reason). Same invalid-value handling as above. |
