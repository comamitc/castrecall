...
- **Health** — `GET {base}/health` → `200` (optionally `{ "status": "ok", "implementation": "whisperx", "model": "large-v3" }`) means ready; any non-2xx or network failure means not ready. Used by `castrecall_setup`/`castrecall_setup_status`; never billed, never throws.
...
- **`remote-stt` shows "off — CASTRECALL_REMOTE_STT_BASE_URL is not set"** — point it at your self-hosted service; `castrecall_setup_status` also probes `GET {base}/health` once enabled and reports "remote NOT ready" with the reason when the service itself is down or rejecting the token.
