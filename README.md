...
**Choosing cloud STT vs. local Whisper vs. remote STT (issue #64).** These are three tiers of the same "what if no free rung produces a transcript" fallback, trading off cost, privacy, and infrastructure:

- **Cloud STT** (`CASTRECALL_STT_PROVIDER=assemblyai`/`openai`/`deepgram`) — zero infrastructure, provider-hosted, billed per episode. Right choice when you don't want to run or maintain anything.
- **Local Whisper** — free and fully private, but bounded by the CPU/GPU of the single machine OpenClaw runs on. Right choice for occasional/low-volume transcription on a laptop or workstation you already have — see "Choosing a backend by platform" above.
- **Remote STT** (`CASTRECALL_STT_PROVIDER=remote-stt`) — private and high-quality like local Whisper, but the transcription runs on **your own** always-on hardware instead of the machine OpenClaw happens to be running on, so multiple OpenClaw hosts (or a laptop with no GPU) can share one beefier box. This is the advanced, reusable option below: the cost is standing up and securing a small service, not a specific GPU. `worker/whisperx/` is one reference worker for it (issue #62, NVIDIA/CUDA), not the only implementation this contract supports — any service speaking the contract below works.
...
Failures classify the same way as the cloud providers: a rejected request or a `429`/`408`/5xx status is retryable (the episode stays eligible for a later run); `401` means the token was rejected; any other non-OK status, a body-level `"failed"` job, or the poll deadline elapsing are terminal. Every transcript's `provenance.json` records the remote `implementation`/`model` and the base URL's **host only** — never the token or the full base URL/path — see [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#provenancegeneration-fields-remote-stt-only-issue-61) for the full field list.

**Security (issue #64).** A remote-stt endpoint is a private service you run, not a public API — treat it like any other credentialed internal endpoint: send `CASTRECALL_REMOTE_STT_TOKEN` and keep the service off the public internet, reachable only over a private network or Tailscale, with no public bind by default. See `worker/whisperx/README.md`'s ["Network exposure (private network / Tailscale)"](worker/whisperx/README.md#network-exposure-private-network--tailscale) section for the concrete loopback-bind-plus-tunnel pattern the reference worker uses — the same shape applies to any self-hosted implementation of this contract, not just WhisperX.

**Async jobs & runtime (issue #64).** When the submit response is a `job_id` (async — see "Response" above), CastRecall polls `GET {base}/jobs/{job_id}` every **5 seconds** for up to a **15-minute** deadline before treating the job as terminal (see the timeout troubleshooting entry below). Polling is durably resumable: before the first poll, the job id is persisted to `<dataDir>/.staging/remote-stt-jobs/`, keyed by base URL + model + submit mode, so a CastRecall restart mid-job resumes polling the existing job instead of re-submitting the audio. Runtime is GPU-bound and depends entirely on your own hardware and model size — as a rough estimate (the same "rough estimate" caveat as the local-Whisper preflight below), expect low minutes per episode on a modern GPU; long episodes on a loaded or modest GPU may approach the 15-minute deadline. See "Corpus-scale transcription preflight" below for the same caveat applied to local Whisper's runtime estimate.
...
Considering remote-stt as an alternative to local Whisper? A corpus-scale run blocked on local Whisper's low-quality-model guardrail also blocks remote-stt (and every other cloud STT provider) for that run rather than silently falling through to it — see "Corpus-scale transcription preflight" below.
...
- **`remote-stt` reachable but reports "degraded" or a capability mismatch** — `castrecall_setup_status`'s `remoteStt` object names the reason: `model_ready: false` (the service is still loading its model — wait and retry), the service self-reporting `"status": "degraded"`, or `accepts` naming only the submit mode you're *not* configured for (e.g. the worker only accepts `upload` but `CASTRECALL_REMOTE_STT_UPLOAD` is unset/`false`, or vice versa). Align `CASTRECALL_REMOTE_STT_UPLOAD` with the worker's advertised `accepts` value — see "Setup/status checks (issue #63)" above for the full state list.
