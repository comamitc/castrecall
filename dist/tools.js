...
export async function setupStatus(config, deps = {}) {
    ...
    const remoteSttStatus = stt.ok && config.stt.provider === "remote-stt" ? await remoteSttHealth(config, deps.fetchImpl) : undefined;
...
export async function setup(config, params = {}, deps = {}) {
    ...
    const remoteStt = sttForSetup.ok && config.stt.provider === "remote-stt"
...
export async function syncHistory(config, params, deps = {}) {
    ...
    const storage = storageFor(config);
    await storage.init();
    const { added, totalSeen } = await storage.recordListens(eligible, deps.now ?? (() => new Date()));
    return {
...
export async function fetchTranscript(config, params, deps = {}) {
    ...
    if (!result.transcript) {
        ...
        if (preflightBlocked) {
            ...
            if (ageMs > TRANSCRIPT_RECHECK_MAX_AGE_MS) {
                ...
                const delay = Math.min(TRANSCRIPT_RECHECK_BASE_MS * 2 ** (attempts - 1), TRANSCRIPT_RECHECK_CAP_MS);
                recheck = {
                    attempts,
                    nextEligibleAt: new Date(now().getTime() + delay).toISOString(),
                    firstDeferredAt,
    ...
    if (loop.looped) {
        ...
        return {
            ...
            loop,
            ladder: result.rungs,
            hint: "Loop-corrupted output was quarantined, not stored — it will not be searched, exported, or " +
    ...
    const correctedSegments = compiledGlossary && result.transcript.segments
 ...
 */
export async function digest(config, params, deps = {}) {
    const storage = storageFor(config);
    const state = await storage.loadState();
    const now = deps.now ?? (() => new Date());
    const nowDate = now();
    ...
    const inWindow = Object.values(state.episodes).filter((record) => Date.parse(record.firstSeenAt) >= windowStart.getTime());
