...
import { isRetryableHttpStatus, PollDeadlineError, RetryableSttError, utterancesToSegments } from "./stt.js";
 ...
 */
function malformedHealthShapeReason(body) {
...
export async function remoteSttHealth(config, fetchImpl = fetch) {
    ...
    try {
        ...
        try {
            ...
            return {
                state: "degraded",
                reason: "Remote STT health endpoint returned a non-JSON body — verify CASTRECALL_REMOTE_STT_BASE_URL " +
                    "points at the STT service.",
            };
        }
        const shapeReason = malformedHealthShapeReason(parsed);
        if (shapeReason)
            return { state: "degraded", reason: shapeReason };
        const body = parsed;
        ...
        if (accepts === otherMode) {
            return {
                ...ready,
                state: "degraded",
                reason: `Remote STT is configured to submit "${configuredMode}", but the provider only accepts ` +
...
async function writeJobState(statePath, jobId, authFailures = 0) {
    await fs.mkdir(path.dirname(statePath), { recursive: true });
    await fs.writeFile(statePath, JSON.stringify({ jobId, authFailures, savedAt: new Date().toISOString() }), "utf8");
}
/**
...
export async function transcribeWithRemoteStt(config, audioUrl, deps = {}) {
    ...
    if (prior) {
        let resumedBody;
        try {
            resumedBody = await pollJob(base, headers, prior.jobId, fetchImpl, sleep, pollIntervalMs, timeoutMs, now);
        }
        catch (error) {
            if (error instanceof UnknownRemoteJobError) {
                ...
                if (failures >= MAX_RESUME_AUTH_FAILURES) {
                    await clearJobState(statePath);
                    throw new Error(`Remote STT rejected auth for saved job ${prior.jobId} on ${failures} consecutive ` +
