...
export function recurringTopics(transcripts) {
    const counts = new Map();
    for (const text of transcripts) {
        for (const token of tokenize(text)) {
            if (token.length < MIN_TOPIC_LENGTH || STOPWORDS.has(token))
                continue;
 ...
 */
export function notableExcerpts(episodes) {
    const candidates = [];
    episodes.forEach((entry, order) => {
        ...
        if (picks.length === 0)
            return;
        const best = picks.reduce((longest, candidate) => candidate.length > longest.length ? candidate : longest);
        candidates.push({
...
export function listeningPattern(episodes) {
...
export function buildDigest(options) {
    ...
    const transcribedPct = pattern.totalEpisodes > 0 ? Math.round((pattern.transcribedCount / pattern.totalEpisodes) * 100) : 0;
    const lines = [
        "---",
