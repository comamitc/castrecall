...
- [x] #53 loop-safe Whisper decoding options — PR #68 squash-merged, issue closed. Decode controls (language, condition-on-previous-text loop prevention default-off, word timestamps, output format, no-speech/logprob/compression-ratio/hallucination-silence thresholds) mapped per flavor with applied/ignored provenance. Four review rounds: dist drift; word-timestamp false provenance on txt output + custom-command silent guardrail bypass; then the inverse — hallucination threshold is a decode guardrail (applies regardless of stored format, implicitly enabling word-timestamp decode) and custom must surface conditionOnPreviousText for both values. 485 tests green.
...
- [x] #43 timestamped segments — PR #78 squash-merged, issue closed. Clean run (6 transitions, internal fix rounds only). Normalized segments with start/end times persisted through storage and surfaced in corpus export.
...
- [x] #62 WhisperX reference worker — `worker/whisperx/` (Docker-first FastAPI + WhisperX on CUDA, out of `package.json.files`/dist entirely — nothing here is required by normal CastRecall use). Written against `remote-stt.ts` as the wire-contract source of truth, not the issue's `/v1/...` prose: canonical `/health`, `POST /transcribe`, `GET /jobs/{id}` mounted alongside additive `/v1/transcriptions`/`/v1/jobs/{id}`/`/v1/jobs/{id}/result` aliases pointing at the same handlers, so CastRecall (configured against the canonical paths) and the issue's literal wording are both satisfied. Every route (including `/health`) requires a bearer token; the worker refuses to start without `WORKER_TOKEN`. In-memory job store (documented non-durable limitation) with `MAX_ACTIVE_JOBS`/`MAX_QUEUED_JOBS` backpressure (429, which CastRecall already treats as retryable). New `src/transcripts/remote-stt-worker-contract.test.ts` replays committed `worker/whisperx/fixtures/*.json` through the real `transcribeWithRemoteStt` via the existing `fetchImpl` DI seam — CI-gated proof the worker's response shapes actually parse, no Python/GPU needed. 756 Node tests green (+6), 32 pytest tests green (backend mocked via `whisperx_backend.py`'s lazy `import whisperx`, so tests need no GPU/torch/whisperx installed). No dist drift (`.test.ts` excluded from `tsc` emit). Post-merge addendum: one silent crash (session teardown killed the detached run; stranded `implementing` stage needed a label rewind — filed agent-pipeline#382) and three pre-merge hardening rounds (non-global+multicast deny, DNS-rebinding-proof pinning across all validated addresses with per-attempt staging truncation, pre-parse upload size gate) — 55 pytest green.
- [x] release v0.17.0 — PR #86 merged, version 0.16.0→0.17.0 (manifest regenerated), tag on main, GitHub release published, milestone closed (by title).
- [x] #63 remote-endpoint setup/status checks — PR #87 squash-merged, issue closed. Clean run (6 transitions, internal fix rounds only). Remote STT endpoints validated like local Whisper/cloud providers across setup/status.
- [x] #64 remote/self-hosted STT docs — PR #88 squash-merged, issue closed. One 10-hour reviewer HANG (process alive, zero output, engine timeout never concluded the call — filed agent-pipeline#398; killed + relaunched, then clean in 4 minutes).
- [x] release v0.18.0 — PR #89 merged, version 0.17.0→0.18.0 (manifest regenerated), tag on main, GitHub release published, milestone closed. ROADMAP COMPLETE: 20/20 issues, releases v0.1.0–v0.18.0.

## GOAL COMPLETE 2026-07-07

All 20 roadmap issues merged and closed across three tracks (original roadmap, local-Whisper quality #51–#56, transcript quality #41–#46, remote STT #61–#64); 13 releases this goal-session (v0.6.0–v0.18.0) published with milestones closed; main green at 762 tests. Only open item: #8 / release v0.6.1 — maintainer-only ClawHub publish steps (docs/RELEASING.md). Upstream feedback: 9 agent-pipeline issues filed (#377, #382 both already fixed upstream; #383 pre-existing; #387, #389, #390, #391, #393, #398 open), 1 closed as duplicate (#388→#371). Recommendation: update the installed pipeline engine (v1.13.0) to pick up the post-release fixes (#371/#379 stale verdicts, #380 evidence persistence, #386 stranded-stage recovery) before the next goal.
